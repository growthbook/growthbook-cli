...
export default class SavedgroupsGet extends Command {
  static description = 'Get a single saved group by ID'
  ...
  async run(): Promise<void> {
    ...
    } = await this.parse(SavedgroupsGet)
    ...
    const savedGroup = await repo.getSavedGroup(id)

    this.logJson(savedGroup)
  }
}
